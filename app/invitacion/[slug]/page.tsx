...
async function uploadGuestPhoto(file:File|null){if(!data||!file)return;setAlbumUploading(true);setError('');const ext=(file.name.split('.').pop()||'jpg').toLowerCase();const safeExt=['jpg','jpeg','png','webp','heic','heif'].includes(ext)?ext:'jpg';const path=`${data.invitacion.id}/${crypto.randomUUID()}.${safeExt}`;const{error:uploadError}=await supabase.storage.from('guest-album').upload(path,file,{cacheControl:'3600',upsert:false,contentType:file.type||undefined});if(uploadError){setAlbumUploading(false);setError(uploadError.message);return}const{error:metaError}=await supabase.from('album_colaborativo_fotos').insert({invitacion_id:data.invitacion.id,storage_path:path,nombre_archivo:file.name,mime_type:file.type||null,tamano_bytes:file.size});setAlbumUploading(false);if(metaError){setError(metaError.message);return}setAlbumUploaded(true)}
...
return <main data-theme-studio={studioTheme.id} data-button-style={studioTheme.buttonStyle} data-template={templateEngine.id} data-family={templateEngine.family} data-layout={templateEngine.layout} data-typography={templateEngine.typography} data-decoration={templateEngine.decoration} data-hero={templateEngine.hero} data-motion={templateEngine.motion} data-radius={templateEngine.radius} className={`premium-public-invite template-engine theme-${templateEngine.id} layout-${templateEngine.layout} typography-${templateEngine.typography} decoration-${templateEngine.decoration} hero-${templateEngine.hero} motion-${templateEngine.motion} radius-${templateEngine.radius} ${opened||!welcomeEnabled?'invitation-opened':'invitation-locked'}`} style={inviteStyle}>
  ...
  {sectionOrder.map(sectionId=>{
    const sections:Record<TemplateSectionId,ReactNode>={
      ...
      hashtag:showHashtag&&hashtag?<section id="hashtag" className="premium-hashtag-section"><div className="premium-hashtag-card"><span>#</span><p>{hashtagText}</p><h2>{hashtag.startsWith('#')?hashtag:`#${hashtag}`}</h2><small>Úsalo al compartir tus fotos y videos</small></div></section>:null,
      wishes:showWishes?<section id="deseos" className="premium-wishes-section"><div className="premium-wishes-card"><p className="premium-small-title">Buzón de deseos</p><h2>{wishesTitle}</h2><p>{wishesText}</p>{wishSent?<div className="premium-wishes-success">💌 Gracias. Tu mensaje fue enviado a los anfitriones.</div>:<form onSubmit={submitWish}><label><span>Tu nombre</span><input value={wishName} onChange={e=>setWishName(e.target.value)} required maxLength={100}/></label><label><span>Tu mensaje</span><textarea rows={4} value={wishMessage} onChange={e=>setWishMessage(e.target.value)} required maxLength={1200}/></label><button disabled={wishSending}>{wishSending?'Enviando…':'Enviar deseo'}</button></form>}</div></section>:null,
      album:showAlbum?<section id="album-colaborativo" className="premium-album-section"><div className="premium-album-card"><div className="premium-album-icon">▧</div><p className="premium-small-title">Álbum colaborativo</p><h2>{albumTitle}</h2><p>{albumText}</p>{albumUploaded?<div className="premium-album-success">✓ Fotografía recibida. Gracias por compartir este recuerdo.</div>:<label className="premium-album-upload"><input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={e=>void uploadGuestPhoto(e.target.files?.[0]||null)} disabled={albumUploading}/><span>{albumUploading?'Subiendo…':'＋ Subir una fotografía'}</span><small>JPG, PNG, WEBP o HEIC · Máximo definido por la configuración de Supabase</small></label>}<div className="premium-album-privacy">Tus fotografías se guardan en un espacio privado del evento.</div></div></section>:null,
