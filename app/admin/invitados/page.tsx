...
import { EditActionIcon, EyeActionIcon, ShareActionIcon, TrashActionIcon } from '@/components/admin/action-icons';
...
export default function InvitadosPage(){const supabase=useMemo(()=>createClient(),[]);const[inv,setInv]=useState<Invitacion[]>([]);const[items,setItems]=useState<Invitado[]>([]);const[loading,setLoading]=useState(true);const[modal,setModal]=useState(false);const[editing,setEditing]=useState<Invitado|null>(null);const[deleting,setDeleting]=useState<Invitado|null>(null);const[form,setForm]=useState(EMPTY);const[error,setError]=useState('');const[search,setSearch]=useState('');const[filter,setFilter]=useState('todas');const[saving,setSaving]=useState(false);const[sharing,setSharing]=useState<Invitado|null>(null);const[csvImport,setCsvImport]=useState(false);
const load=useCallback(async()=>{setLoading(true);const[a,b]=await Promise.all([supabase.from('invitaciones').select('*').eq('modalidad','pases').order('titulo'),supabase.from('invitados').select('*, invitaciones(id,titulo,slug,estado)').order('created_at',{ascending:false})]);if(a.error)setError(messageFromError(a.error));else setInv((a.data??[]) as Invitacion[]);if(b.error)setError(messageFromError(b.error));else setItems((b.data??[]) as Invitado[]);setLoading(false)},[supabase]);useEffect(()=>{const timer=window.setTimeout(()=>{void load()},0);return()=>window.clearTimeout(timer)},[load]);
...
return <div className="page-stack"><section className="page-heading"><div><p className="eyebrow">Control de acceso</p><h1>Invitados y pases</h1><p>Administra los invitados y sus pases personalizados.</p></div><div className="page-heading-actions"><button className="button button-outline" onClick={()=>setCsvImport(true)}>Importar CSV</button><button className="button button-primary" onClick={openNew}>+ Nuevo invitado</button></div></section><section className="stats-grid guest-stats"><article className="stat-card"><span>Registros</span><strong>{items.length}</strong><small>Familias o invitados</small></article><article className="stat-card"><span>Pases asignados</span><strong>{passes}</strong><small>Adultos y niños</small></article><article className="stat-card"><span>Confirmados</span><strong>{items.filter(x=>x.estado==='confirmado').length}</strong><small>Respondieron sí</small></article><article className="stat-card"><span>Pendientes</span><strong>{items.filter(x=>x.estado==='pendiente').length}</strong><small>Sin respuesta</small></article></section><section className="panel-card"><div className="panel-header events-toolbar"><div><h2>Lista de invitados</h2><p>Genera enlaces privados.</p></div><div className="events-filters"><label className="search-field"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar invitado o código"/></label><select className="status-filter" value={filter} onChange={e=>setFilter(e.target.value)}><option value="todas">Todos</option><option value="pendiente">Pendiente</option><option value="confirmado">Confirmado</option><option value="no_asistira">No asistirá</option></select></div></div>{loading?<div className="dashboard-loading">Cargando invitados…</div>:list.length===0?<EmptyState icon="♚" title="Sin invitados" description="Necesitas una invitación con modalidad pases." />:<div className="table-wrap"><table className="data-table centered-data-table"><thead><tr><th>Invitado</th><th className="table-center">Invitación</th><th className="table-center">Pases</th><th className="table-center">Mesa</th><th className="table-center">Estado</th><th className="table-center">Acciones</th></tr></thead><tbody>{list.map(x=><tr key={x.id}><td><div className="client-name-cell"><span className="client-avatar">{initials(x.nombre)}</span><div><strong>{x.nombre}</strong><span>{x.codigo}</span></div></div></td><td className="table-center">{x.invitaciones?.titulo||'—'}</td><td className="table-center">{x.adultos_permitidos} adultos · {x.ninos_permitidos} niños</td><td className="table-center">{x.mesa||'—'}</td><td className="table-center"><StatusBadge tone={x.estado==='confirmado'?'success':x.estado==='no_asistira'?'danger':'warning'}>{x.estado.replace('_',' ')}</StatusBadge></td><td className="table-center"><ActionGroup columns={4}><ActionButton tone="preview" icon={<EyeActionIcon/>} href={`/invitacion/${x.invitaciones?.slug}/${x.codigo}?preview=1`} target="_blank">Vista previa</ActionButton><ActionButton tone="share" icon={<ShareActionIcon/>} onClick={()=>setSharing(x)}>Compartir</ActionButton><ActionButton tone="edit" icon={<EditActionIcon/>} onClick={()=>openEdit(x)}>Editar</ActionButton><ActionButton tone="delete" icon={<TrashActionIcon/>} onClick={()=>setDeleting(x)}>Eliminar</ActionButton></ActionGroup></td></tr>)}</tbody></table></div>}</section>{error&&!modal&&<p className="form-error">{error}</p>}
...
{csvImport&&<GuestCsvImportModal open={csvImport} invitations={inv} onClose={()=>setCsvImport(false)} onImported={load}/>}
