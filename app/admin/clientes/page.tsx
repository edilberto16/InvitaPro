...
type AuthUser={id:string;email:string;created_at:string;last_sign_in_at:string|null;email_confirmed_at:string|null;is_current_admin:boolean;role:string|null;active:boolean;client:{id:string;nombre:string;correo:string|null;user_id:string}|null};
...
export default function ClientesPage() {
  ...
  async function deleteAuthUser(){ if(!authDelete)return; setAccountBusy(true); const response=await fetch(`/api/admin/accounts/${authDelete.id}`,{method:'DELETE'}); const result=await response.json().catch(()=>({})); setAccountBusy(false); if(!response.ok){setError(result.message||'No fue posible eliminar el usuario de Supabase Auth.');setAuthDelete(null);return;} setAuthDelete(null); await Promise.all([load(),loadAuthUsers()]); }
  ...
  return <div className="page-stack">
    ...
    <section className="panel-card"><div className="panel-header client-toolbar"><div><h2>Usuarios de Supabase Auth</h2><p>Cuentas registradas que existen realmente en Authentication, incluso si todavía no están vinculadas a un cliente.</p></div><button className="button button-ghost" onClick={()=>void loadAuthUsers()} disabled={authLoading}>{authLoading?'Actualizando…':'Actualizar'}</button></div>
      {authLoading&&authUsers.length===0?<div className="dashboard-loading">Consultando Supabase Auth…</div>:authUsers.length===0?<EmptyState icon="♙" title="Sin usuarios de acceso" description="No hay cuentas registradas en Supabase Auth." />:<div className="table-wrap"><table className="data-table centered-data-table"><thead><tr><th>Usuario</th><th className="table-center">Confirmación</th><th className="table-center">Vinculación</th><th className="table-center">Acciones</th></tr></thead><tbody>{authUsers.map(u=><tr key={u.id}><td><strong>{u.client?.nombre||u.email||'Usuario sin correo'}</strong><span>{u.email}</span></td><td className="table-center"><div className="auth-status-stack"><StatusBadge tone={u.email_confirmed_at?'success':'warning'}>{u.email_confirmed_at?'Correo confirmado':'Sin confirmar'}</StatusBadge><StatusBadge tone={u.active?'success':'neutral'}>{u.active?'Cuenta activa':'Suspendida'}</StatusBadge></div></td><td className="table-center">{u.client?<><strong>Cliente vinculado</strong><span>{u.client.nombre}</span></>:<><strong>Solo Auth</strong><span>{u.role?`Rol: ${u.role}`:'Sin perfil vinculado'}</span></>}</td><td className="table-center">{u.is_current_admin?<StatusBadge tone="neutral">Tu sesión admin</StatusBadge>:<ActionButton tone="delete" icon={<TrashActionIcon/>} onClick={()=>setAuthDelete(u)}>Eliminar de Auth</ActionButton>}</td></tr>)}</tbody></table></div>}
    </section>
    ...
    <ConfirmDialog open={Boolean(authDelete)} eyebrow="Supabase Authentication" title={`¿Eliminar ${authDelete?.email || 'este usuario'}?`} description="Esta acción elimina definitivamente la cuenta de Supabase Auth. Si existe un registro comercial de cliente, se conserva para no perder eventos ni invitaciones." confirmLabel="Eliminar de Supabase Auth" busy={accountBusy} onCancel={()=>!accountBusy&&setAuthDelete(null)} onConfirm={deleteAuthUser}/>
