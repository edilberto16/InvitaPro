...
export default function InvitacionesPage(){const supabase=useMemo(()=>createClient(),[]);const[eventos,setEventos]=useState<Evento[]>([]);const[items,setItems]=useState<Invitacion[]>([]);const[loading,setLoading]=useState(true);const[modal,setModal]=useState(false);const[editing,setEditing]=useState<Invitacion|null>(null);const[deleting,setDeleting]=useState<Invitacion|null>(null);const[form,setForm]=useState<FormState>(EMPTY);const[error,setError]=useState('');const[search,setSearch]=useState('');const[filter,setFilter]=useState('todas');const[saving,setSaving]=useState(false);const[uploading,setUploading]=useState<'cover'|'gallery'|'audio'|null>(null);const[sharing,setSharing]=useState<Invitacion|null>(null);const[mediaPicker,setMediaPicker]=useState<'cover'|'gallery'|'audio'|null>(null);const[reviewing,setReviewing]=useState<Invitacion|null>(null);const[reviewBusy,setReviewBusy]=useState(false);const[templateFilter,setTemplateFilter]=useState('todas');const[copiedSlug,setCopiedSlug]=useState('');const[activating,setActivating]=useState<Invitacion|null>(null);const[activationBusy,setActivationBusy]=useState(false);const[draggedSection,setDraggedSection]=useState<TemplateSectionId|null>(null);const[selectedSection,setSelectedSection]=useState<TemplateSectionId>('hero');
...
async function activateInvitation(){
  ...
  const {error}=await supabase.from('invitaciones').update({
    ...
    design_json:{...current,activation_status:'approved',activation_approved_at:new Date().toISOString(),activation_payment_status:'confirmado'}
  }).eq('id',activating.id);
