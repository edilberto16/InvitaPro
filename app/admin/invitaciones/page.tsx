...
import { canUseInvitationModality, invitationModalityLabel, normalizeInvitationModality } from '@/lib/invitation-modality';
...
const DEFAULT_SECTION_SETTINGS:SectionSettings={
  ...
  history:{eyebrow:'Un capítulo especial',title:'Nuestra historia',description:'',buttonLabel:'',alignment:'center'},
  lodging:{eyebrow:'Para tu comodidad',title:'Hospedaje recomendado',description:'Opciones para que disfrutes el evento sin preocupaciones.',buttonLabel:'',alignment:'center'},
...
function sectionEnabled(sectionId:TemplateSectionId){
  ...
  const visibilityField:Partial<Record<TemplateSectionId,keyof FormState>>={
    ...
    details:'mostrar_detalles',
    program:'mostrar_programa',
    gallery:'mostrar_galeria',
    location:'mostrar_mapa',
    rsvp:'mostrar_rsvp',
  };

  const field=visibilityField[sectionId];
...
async function save(e:FormEvent){e.preventDefault();const slug=slugify(form.slug||form.titulo);if(!form.evento_id)return setError('Selecciona un evento.');if(!form.titulo.trim()||!slug)return setError('Título y enlace son obligatorios.');const selectedCommercialPlan=planByKey(commercialPlans,form.plan_clave);if(!canUseInvitationModality(selectedCommercialPlan,form.modalidad))return setError(form.modalidad==='pases'?'Pases personalizados requiere Premium o Signature.':'El plan seleccionado no incluye RSVP.');
if(editing&&normalizeInvitationModality(editing.modalidad)!==normalizeInvitationModality(form.modalidad)){
  const [{count:confirmationCount,error:confirmationError},{data:guestRows,error:guestError}]=await Promise.all([
    supabase.from('confirmaciones').select('id',{count:'exact',head:true}).eq('invitacion_id',editing.id),
    supabase.from('invitados').select('id,checkin_at,checkin_count,checkin_adultos,checkin_ninos').eq('invitacion_id',editing.id)
  ]);
  if(confirmationError||guestError)return setError(confirmationError?.message||guestError?.message||'No fue posible validar el cambio de modalidad.');
  const hasGuestActivity=(guestRows||[]).some(row=>Boolean(row.checkin_at)||Number(row.checkin_count||0)>0||Number(row.checkin_adultos||0)>0||Number(row.checkin_ninos||0)>0);
  const affected=(confirmationCount||0)>0||(guestRows||[]).length>0;
  if(affected){
    const warning=hasGuestActivity
      ? 'Esta invitación ya tiene invitados y registros de check-in. Cambiar la modalidad puede dejar funciones o enlaces individuales fuera de uso. ¿Deseas continuar?'
      : 'Esta invitación ya tiene invitados o confirmaciones. Cambiar la modalidad modificará lo que el cliente puede gestionar. ¿Deseas continuar?';
    if(!window.confirm(warning))return;
  }
}
setSaving(true);const design_json={version:7,commercial_plan_key:selectedCommercialPlan.clave,modality_key:normalizeInvitationModality(form.modalidad),modality_label:invitationModalityLabel(form.modalidad),commercial_plan_name:selectedCommercialPlan.nombre,commercial_plan_price_snapshot:selectedCommercialPlan.precio_mxn,componentes:[],theme_id:form.theme_id,theme_overrides:form.theme_overrides,section_order:form.section_order,section_settings:form.section_settings,mostrar_intro:form.mostrar_intro,plantilla:form.plantilla,mensaje:form.mensaje.trim(),subtitulo:form.subtitulo.trim(),vestimenta:form.vestimenta.trim(),programa:form.programa.trim(),portada_url:form.portada_url,portada_efecto:form.portada_efecto,pantalla_bienvenida:form.pantalla_bienvenida,texto_bienvenida:form.texto_bienvenida.trim()||'Abrir invitación',galeria_urls:form.galeria_urls,mostrar_contador:form.mostrar_contador,mostrar_detalles:form.mostrar_detalles,mostrar_programa:form.mostrar_programa,mostrar_galeria:form.mostrar_galeria,mostrar_mapa:form.mostrar_mapa,mostrar_rsvp:form.mostrar_rsvp};const payload={titulo:form.titulo.trim(),slug,modalidad:form.modalidad,estado:form.estado,design_json,color_principal:form.color_principal,musica_url:form.musica_url.trim()||null,whatsapp:form.whatsapp.replace(/\D/g,'')||null,fecha_publicacion:form.estado==='publicada'?(editing?.fecha_publicacion??new Date().toISOString()):null,fecha_expiracion:form.fecha_expiracion?new Date(form.fecha_expiracion).toISOString():null};const r=editing?await supabase.from('invitaciones').update(payload).eq('id',editing.id):await supabase.from('invitaciones').insert({...payload,evento_id:form.evento_id});setSaving(false);if(r.error)return setError(messageFromError(r.error));setModal(false);await load()}
...
const statusCounts=useMemo(()=>({
  ...
  pendiente_activacion:items.filter(x=>x.estado==='pendiente_activacion').length,
...
async function activateInvitation(){
  ...
  const current=activating.design_json||{};
  const planKey=activationPlan(activating);
  const planName=planLabel(planKey);
  const listPrice=Number(current.activation_price_snapshot||0);
  const paid=activationCourtesy?0:Number(activationAmount||0);
  const {data:plan}=await supabase.from('planes_comerciales').select('id,nombre,precio_mxn').eq('clave',planKey).maybeSingle();
  const sale=await supabase.from('ventas_invitaciones').upsert({
    invitacion_id:activating.id,
    plan_id:plan?.id||null,
    plan_clave:planKey,
    plan_nombre:plan?.nombre||planName,
    precio_lista:listPrice||Number(plan?.precio_mxn||0),
    importe_pagado:paid,
    metodo_pago:activationCourtesy?'cortesia':activationMethod,
    estado_pago:activationCourtesy?'cortesia':'confirmado',
    referencia:activationReference.trim()||null,
    notas:activationNotes.trim()||null,
    activated_at:new Date().toISOString(),
    updated_at:new Date().toISOString()
  },{onConflict:'invitacion_id'});
  if(sale.error){setActivationBusy(false);setError(messageFromError(sale.error));return;}
  const {error}=await supabase.from('invitaciones').update({
    estado:'publicada',
    fecha_publicacion:activating.fecha_publicacion??new Date().toISOString(),
    design_json:{...current,activation_status:'approved',activation_approved_at:new Date().toISOString(),activation_payment_status:activationCourtesy?'cortesia':'confirmado',activation_amount_paid:paid,activation_method:activationCourtesy?'cortesia':activationMethod}
  }).eq('id',activating.id);
  setActivationBusy(false);
  if(error){setError(messageFromError(error));return;}
  setActivating(null);
  await load();
}
        ...
        <section className="invitation-builder-section">
          ...
          <div className="commercial-plan-selector">
            ...
            <div className="commercial-plan-selector-grid">
              {commercialPlans.map(plan=><button key={plan.clave} type="button" className={form.plan_clave===plan.clave?'selected':''} onClick={()=>setForm(current=>{const nextModality=canUseInvitationModality(plan,current.modalidad)?current.modalidad:(plan.permite_rsvp?'rsvp':'simple');return {...current,plan_clave:plan.clave,modalidad:nextModality}})}><span>{plan.nombre}</span><strong>{moneyMXN(plan.precio_mxn)}</strong><small>{plan.descripcion}</small></button>)}
            </div>
          </div>
          ...
          <div className="modality-card-grid">
            {MODALITIES.map(option=>{
              ...
              const locked=!canUseInvitationModality(currentPlan,option.id);
            ...
            <section className="invitation-builder-section">
              ...
              <div className="invitation-form-grid">
                <label className="form-field">
                  <span>Evento *</span>
                  <select value={form.evento_id} disabled={Boolean(editing)} onChange={e=>{const ev=eventos.find(x=>x.id===e.target.value);setForm({...form,evento_id:e.target.value,titulo:form.titulo||ev?.nombre||'',slug:form.slug||slugify(ev?.nombre||'')})}}>
...
{activating&&<div className="modal-backdrop activation-admin-backdrop" onMouseDown={()=>!activationBusy&&setActivating(null)}>
  <section className="activation-admin-modal" onMouseDown={e=>e.stopPropagation()}>
    <header>
      <div><p className="eyebrow">Activación comercial</p><h2>Revisar y publicar</h2><p>Confirma el plan y publica el enlace definitivo del cliente.</p></div>
      <button type="button" className="modal-close" disabled={activationBusy} onClick={()=>setActivating(null)}>×</button>
    </header>
    <div className="activation-admin-grid">
      <section className="activation-admin-summary">
        <div className="activation-plan-card"><small>PLAN SOLICITADO</small><strong>{planLabel(activationPlan(activating))}</strong><span>{activationPlan(activating)==='signature'?'Experiencia exclusiva':activationPlan(activating)==='premium'?'Experiencia completa':'Invitación esencial'}</span></div>
        <dl>
          <div><dt>Invitación</dt><dd>{activating.titulo}</dd></div>
          <div><dt>Cliente</dt><dd>{activating.eventos?.clientes?.nombre||'—'}</dd></div>
          <div><dt>Evento</dt><dd>{activating.eventos?.nombre||'—'}</dd></div>
          <div><dt>Fecha</dt><dd>{formatDate(activating.eventos?.fecha)}</dd></div>
          <div><dt>URL pública</dt><dd>/invitacion/{activating.slug}</dd></div>
        </dl>
      </section>
      <section className="activation-admin-checklist">
        ...
        <div className="activation-sale-fields">
          <label><span>Importe cobrado</span><input type="number" min="0" step="1" value={activationAmount} onChange={e=>setActivationAmount(e.target.value)} disabled={activationCourtesy}/></label>
          <label><span>Método de pago</span><select value={activationMethod} onChange={e=>setActivationMethod(e.target.value)} disabled={activationCourtesy}><option value="transferencia">Transferencia</option><option value="efectivo">Efectivo</option><option value="tarjeta">Tarjeta</option><option value="otro">Otro</option></select></label>
          <label className="full"><span>Referencia</span><input value={activationReference} onChange={e=>setActivationReference(e.target.value)} placeholder="Folio, últimos 4 dígitos, referencia…"/></label>
          <label className="full activation-courtesy"><input type="checkbox" checked={activationCourtesy} onChange={e=>setActivationCourtesy(e.target.checked)}/><span>Activar como cortesía</span></label>
          <label className="full"><span>Notas internas</span><textarea rows={2} value={activationNotes} onChange={e=>setActivationNotes(e.target.value)} placeholder="Opcional"/></label>
        </div><div className="activation-admin-note"><strong>Al activar:</strong><p>La invitación cambiará a Publicada y el cliente podrá copiar y compartir el enlace definitivo.</p></div>
      </section>
    </div>
    <footer>
      <Link className="button button-ghost" href={`/invitacion/${activating.slug}?preview=1`} target="_blank"><EyeIcon/><span>Vista previa</span></Link>
      <div>
        <button type="button" className="button button-outline" disabled={activationBusy} onClick={()=>setActivating(null)}>Cancelar</button>
        <button type="button" className="button button-success activation-confirm-button" disabled={activationBusy} onClick={()=>void activateInvitation()}>{activationBusy?'Activando…':'✓ Confirmar y publicar'}</button>
      </div>
    </footer>
  </section>
</div>}
