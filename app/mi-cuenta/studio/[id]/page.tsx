...
import {CommercialPlan,DEFAULT_COMMERCIAL_PLANS,commercialPlanBenefits,moneyMXN,planByKey,resolveInvitationCommercialPlanKey} from "@/lib/commercial-plans";
...
export default function StudioPage(){
 ...
 function previewTheme(themeIdValue:string|null){
  setThemePreviewId(themeIdValue);
 }
 ...
 return <main className="studio-page">{templateNotice&&<div className="studio-template-toast">{templateNotice}</div>}
  ...
  {showThemes&&<StudioThemeSelector value={themeId} onPreview={previewTheme} onChange={applyTheme} onClose={()=>{setThemePreviewId(null);setShowThemes(false)}}/>}
  {showPublish&&<div className="modal-backdrop activation-backdrop" onMouseDown={()=>!requestingActivation&&setShowPublish(false)}><section className="activation-modal" onMouseDown={e=>e.stopPropagation()}>
   ...
   <div className="activation-plans">{commercialPlans.filter(p=>p.activo).map(plan=>{const isSelected=selectedPlan===plan.clave;const isCurrent=currentPlanKey===plan.clave;return <button key={plan.clave} className={`${isSelected?"selected":""} ${plan.clave==="premium"?"featured":""} ${isCurrent?"current":""}`.trim()} onClick={()=>{setSelectedPlan(plan.clave);setActivationIssues([])}}><span>{plan.nombre.toUpperCase()}</span><strong>{moneyMXN(plan.precio_mxn)}</strong><small>{plan.descripcion}</small><ul>{commercialPlanBenefits(plan).map(feature=><li key={feature}>✓ {feature}</li>)}</ul>{isSelected?<em>Plan seleccionado</em>:isCurrent?<em>Plan actual</em>:plan.clave==="premium"?<em>Más elegido</em>:null}</button>})}</div>
   <div className="activation-summary"><div><small>EVENTO</small><strong>{title||invite.titulo}</strong><span>{date||"Fecha por definir"} · {venue||"Ubicación por definir"}</span></div><div><small>PLAN SELECCIONADO</small><strong>{planByKey(commercialPlans,selectedPlan).nombre}</strong><span>{moneyMXN(planByKey(commercialPlans,selectedPlan).precio_mxn)} · Pago por transferencia · Activación manual</span><div className="activation-summary-benefits">{commercialPlanBenefits(planByKey(commercialPlans,selectedPlan)).map(feature=><i key={feature}>✓ {feature}</i>)}</div></div></div>
   ...
   <footer><button className="client-secondary" disabled={requestingActivation} onClick={()=>setShowPublish(false)}>Seguir editando</button><button className="client-primary activation-submit" disabled={requestingActivation||!title.trim()||!date||!invite.template_key} onClick={()=>void requestActivation()}>{requestingActivation?"Enviando…":`Solicitar activación de ${planByKey(commercialPlans,selectedPlan).nombre} →`}</button></footer>
   ...
   <section className="studio-template-modal studio-template-modal-global" onMouseDown={e=>e.stopPropagation()}>
      ...
      {templates.map(t=>{const allowed=canUseTemplate(t,currentPlanKey);return <article key={t.id} className={`studio-global-template-card ${invite.template_key===t.id?"selected":""} ${allowed?"":"template-locked"}`}>
        ...
        <div className="studio-global-template-info">
          ...
          <div className="studio-global-template-actions">
            <Link className="client-secondary" target="_blank" href={`/preview/plantilla?tipo=${t.collection}&plantilla=${t.id}&origen=cliente&context=change&eventId=${encodeURIComponent(invite.id)}&returnTo=${encodeURIComponent(`/mi-cuenta/studio/${invite.id}`)}`}>Ver demo completa</Link>
    ...
    <section className="template-confirm-modal" onMouseDown={e=>e.stopPropagation()}>
      ...
      <footer className="template-confirm-actions">
        <Link className="client-secondary" target="_blank" href={`/preview/plantilla?tipo=${selected.collection}&plantilla=${selected.id}&origen=cliente&context=change&eventId=${encodeURIComponent(invite.id)}&returnTo=${encodeURIComponent(`/mi-cuenta/studio/${invite.id}`)}`}>Ver demo</Link>
